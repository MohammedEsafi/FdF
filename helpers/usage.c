...
void	usage(t_fdf fdf)
{
	...
	mlx_string_put(mlx_ptr, win_ptr, PAD, 4 * LH, WH, "Move: Arrows");
	...
	mlx_string_put(mlx_ptr, win_ptr, PAD, 17 * LH, WH, "Initialization: 0 Key");
	mlx_string_put(mlx_ptr, win_ptr, PAD, 18 * LH, WH, "Exit: ESC Key");
}
